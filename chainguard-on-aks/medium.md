...
converts #, ##, **bold**, `code`, ``` blocks and > quotes on paste. Images
load from the URLs; the YouTube link on its own line becomes an embed.
Then set Story settings > Advanced > canonical link to the dev.to URL.

Tags (Medium allows 5): Kubernetes, Azure, Container Security,
Supply Chain Security, DevOps
-->

# Same app, two base images: 178 CVEs vs 5. A hands-on Chainguard on AKS walkthrough

*Step by step: build one FastAPI app on python:3.13-slim and on a Chainguard image, scan, verify signatures, ship SBOMs, deploy to AKS behind a Kyverno policy, and let an AI agent skill do the migration.*

![Cover](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/cover.png)
...
https://www.youtube.com/watch?v=-dMyVMPeUug
...
- **Base image:** `python:3.13-slim` versus `cgr.dev/chainguard/python`
- **User:** root versus nonroot (uid 65532)
- **Shell and package manager:** present versus absent
...
![docker images: 248MB vs 140MB](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/02-images.png)
...
![grype output for both images](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/04-grype.png)

A small script summarises the two JSON reports into one table:

![comparison table: 178 CVEs vs 5](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/01-compare.png)
...
![upstream: root with apt-get and 259 binaries; chainguard: no sh at all](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/03-shell.png)
...
![cosign verify output showing issuer and identity](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/05-verify.png)
...
![syft SBOM package counts, 109 vs 47](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/06-sbom.png)
...
![upstream app page: Debian, uid 0, shell present, 87 packages](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/07-app-upstream.png)

![chainguard app page: Wolfi, uid 65532, no shell, 26 packages](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/08-app-chainguard.png)

And the one-page report the scan script generates, useful for a slide:

![HTML report comparing both images](https://raw.githubusercontent.com/sathpal/azure-articles/main/chainguard-on-aks/img/09-report.png)
