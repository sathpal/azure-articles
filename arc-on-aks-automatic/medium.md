...
Preferred: publish on dev.to first, then Medium > "Import a story" with the dev.to
URL. Medium imports the rendered HTML, images and code blocks and sets the canonical
link to dev.to for you.

Manual: paste everything below into the Medium editor. #, ##, **bold**, `code`,
``` blocks and images by URL all convert on paste. Then set the canonical link
under Story settings > Advanced to the dev.to URL.
...
![Cover](https://raw.githubusercontent.com/sathpal/azure-articles/main/arc-on-aks-automatic/img/cover.png)
